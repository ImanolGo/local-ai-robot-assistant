...
class MonodepthOptions:
    def __init__(self):
        ...
        self.parser.add_argument(
            "--data_path", type=str, help="path to the training data", required=True
        )
        ...
        self.parser.add_argument(
            "--disparity_smoothness", type=float, help="disparity smoothness weight", default=1e-3
        )
        self.parser.add_argument(
            "--scales", nargs="+", type=int, help="scales used in the loss", default=[0, 1, 2, 3]
        )
        #   default=[0, 1, 2])
        ...
        self.parser.add_argument(
            "--use_stereo", help="if set, uses stereo pair for training", action="store_true"
        )
        self.parser.add_argument(
        ...
        # ABLATION options
        self.parser.add_argument(
            "--v1_multiscale", help="if set, uses monodepth v1 multiscale", action="store_true"
        )
        self.parser.add_argument(
            "--avg_reprojection", help="if set, uses average reprojection loss", action="store_true"
        )
        self.parser.add_argument(
            "--disable_automasking", help="if set, doesn't do auto-masking", action="store_true"
        )
        self.parser.add_argument(
            "--predictive_mask",
        ...
        self.parser.add_argument(
            "--use_learnable_K",
            help="if set, add a new learnable parameter for the focal lengths "
            " (horizontal/vertical) and the center, it should work reasonably "
            " well if the camera intrinsics aren't well known or you only have"
            " a rough estimate of the fov/focal lengths.",
            action="store_true",
        )
        self.parser.add_argument(
            "--use_feature_reprojection_loss",
            help="if set, add the feature reprojection loss to the loss       "
            " function. It is modified from 'Unsupervised Learning of         "
            " Monocular Depth Estimation and Visual Odometry with Deep Feature"
            " Reconstruction'",
            action="store_true",
        )
        ...
        self.parser.add_argument(
            "--save_frequency", type=int, help="number of epochs between each save", default=1
        )
        ...
        self.parser.add_argument(
            "--save_pred_disps", help="if set saves predicted disparities", action="store_true"
        )
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            "--eval_out_dir", help="if set will output the disparities to this folder", type=str
        )
        self.parser.add_argument(
            "--post_process",
