...
9. **Wave Rover has no wheel encoders + IMU bias**: `/chassis_state.left_motor_current`/`right_motor_current` are parsed from T=1001 `L`/`R`, which are always 0 on the encoder-less Wave Rover; the base IMU also carries a large constant gyro bias (~0.36 rad/s). Motion therefore cannot be measured from wheel telemetry.
   - Status: ✅ Mitigated: `/odom_raw` is published as dead-reckoning from commanded wheel speeds; a `motor.min_wheel_pwm` deadband floor (default 0.3) makes low-speed turns actually move. Direct-serial gyro sweeps confirmed the drive deadband (~0.25–0.3 normalized, i.e. ~50–60% PWM).

---

## Recent Updates

- **25 Sep 2026**: **Motor check on charged hardware + deadband fix.** Re-verified the full path with the pack charged (12.2 V): serial opens, T=1001 feedback + IMU stream, `/cmd_vel` is written as `{"T": 1, ...}`, `/emergency_stop` activates/releases, and `/odom_raw` publishes. Measured the drivetrain deadband directly over serial with a bias-corrected gyro: no rotation below `|L|=|R|≈0.25`, starts ~0.3, strong at 0.4 — so standard low-speed turns (0.5 rad/s → ±0.067) did nothing. Added `motor.min_wheel_pwm` (default 0.3) deadband compensation and refactored the mapping into a pure `twist_to_wheel_speeds()` with unit tests, plus `hardware_tests/test_motor_ros.py`. Note: the Wave Rover has **no encoders**, so T=1001 `L`/`R` are always 0 and the IMU has a ~0.36 rad/s gyro bias; odometry is dead-reckoning. Test suite: **103 passed, 3 failed** (pre-existing).

- **25 Sep 2026**: **Motor check + odometry.** Verified the UART path end-to-end on hardware: `/dev/ttyTHS1` opens, continuous feedback (T=1001) and IMU stream, `/cmd_vel` is translated and written as `{"T": 1, "L": ..., "R": ...}` frames, and the `/emergency_stop` service activates/releases correctly. Found that `/odom_raw` was advertised but never published; implemented 20 Hz dead-reckoning odometry (differential drive from commanded wheel speeds, `odom`→`base_link`) plus unit tests. Observation while commanding motion: the firmware reported `L=R=0` and the IMU yaw did not change, with no battery voltage sag — i.e. the wheels did not draw current (likely the motor power switch is off or the pack is low at ~10.04 V), so physical motion could not be confirmed.
